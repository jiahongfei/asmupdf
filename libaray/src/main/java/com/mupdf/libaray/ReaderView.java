...
public class ReaderView
	...
	public boolean onDoubleScale(float scale, float centerX, float centerY) {
		...
		mScale = Math.min(Math.max(mScale * scale, min_scale), max_scale);
		...
		if (mReflow) {
			...
			if (v != null) {
				...
				int viewFocusX = (int)centerX - (v.getLeft() + mXScroll);
				int viewFocusY = (int)centerY - (v.getTop() + mYScroll);
